...
export default {
  ...
  methods: {
    ...
    BarChart(
      ...
      axisY.selectAll(".tick_line").transition().duration(400).attr("opacity", 1);
      axisY.selectAll(".tick_long_line").transition().duration(400).attr("opacity", 1);
      ...
      const bars = svg
        ...
        .attr("class", (d, i) => (Y[i] >= 0 ? "bar bar_positive" : "bar bar_negative"))
        .attr("height", 0)
      ...
      if (isShowNumber) {
        ...
        const numbers = svg
          ...
          .attr("opacity", "0")
          .text((i) => Y[i]+( typeof numberSuffix == "function" ? suffixArr[i] : numberSuffix));
        ...
        numbers
          ...
          .duration(duration)
          .ease(d3[ease + "Out"])
          .attr("y", (i) => yScale(isNegativeSameDirection ? Math.abs(Y[i]) : Y[i]))
          .attr("opacity", "1");
      }
